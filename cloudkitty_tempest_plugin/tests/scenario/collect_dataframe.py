...
class DataFrameCollectionScenarioTest(base.BaseRatingTest): #should I be inheriting from scenario test instead?  gives access to create_volume function


#could create volume client?
``
...
def _setup_volume_resource(self):
    ...
    # create field mapping for the volume resource
    # add details to the mapping (cost, flat rate, etc)

    #must work out if i need to pass in the values that are currently none
